 ...
 * 
 * @returns {string} "Ok" o "Error: <codigo> - <mensaje"
 */
export const loadDefaultsRetail = async (id, setValue, setCasoActual, formatDate) => {
    ...
    if ( !id ) {
        setValue("nroCaso", 0);
        setValue("mail", "")  // tomarlo del usuario
        setCasoActual({mdo: "Alta", id: "", idCRM: "N/D", mail: "mail@mail.com", 
                        items: [{id: "", casoId: "", fila: 0, 
                                producto: {id: "", idERP: "", name: "", tipo: ""}}]});
        return "Ok"
    } 
    const casoData = await Caso.GetById(id);
    console.log("loadDefaultsRetail-casoData: ", casoData);
    if (!casoData.status){
